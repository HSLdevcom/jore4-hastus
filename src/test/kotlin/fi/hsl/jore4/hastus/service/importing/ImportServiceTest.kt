...
class ImportServiceTest(
    ...
    fun `when the start date of the booking record is earlier than the route of the related journey pattern`() {
        ...
        val exception = assertFailsWith<CannotFindJourneyPatternRefByRouteLabelAndDirectionException> {
    ...
    @Test
    fun `when the stop labels do not match between Hastus trip and existing journey-pattern-ref`() {
    ...
    @Test
    fun `when the timing place labels do not match between Hastus trip and existing journey-pattern-ref`() {
        populateDatabaseFromDataset("datasets/journey_pattern_ref_100_with_one_timing_place_label_difference.json")

        val csvToImport: String = readFileAsString("hastus_booking_records/100_both_directions.exp")

        val exception = assertFailsWith<CannotFindJourneyPatternRefByTimingPlaceLabelsException> {
            service.importTimetablesFromCsv(csvToImport, getHasuraHeaders())
        }
